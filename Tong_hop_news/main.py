...
KEYWORDS = {
    "Thể thao": ["bóng đá", "v-league", "hlv", "cầu thủ", "đội tuyển", "seagames", "world cup", "thể thao", "trận đấu", "bàn thắng", "cup", "messi", "ronaldo"],
    "Giáo dục": ["học sinh", "sinh viên", "đại học", "giáo dục", "trường học", "thi tốt nghiệp", "bộ giáo dục", "thầy cô", "học phí", "tuyển sinh"],
    "Pháp luật": ["công an", "bắt giữ", "khởi tố", "tội phạm", "án mạng", "tòa án", "vi phạm", "trộm cắp", "điều tra", "cảnh sát", "tù", "xét xử"],
    "Kinh doanh": ["chứng khoán", "ngân hàng", "tỷ giá", "doanh nghiệp", "kinh tế", "lãi suất", "thị trường", "vàng", "bất động sản", "xăng dầu", "giá vàng", "usd"],
...
def classify_news(title):
    """Hàm tự động đoán chủ đề bài báo dựa trên tiêu đề"""
...
def collect():
    ...
    for src_name, func in [("VNExpress", vne), ("24h", haitu), ("Tuổi Trẻ", tt)]:
        ...
        try:
            articles = func() 
            print(f"   -> Lấy được {len(articles)} tin.")
            for art in articles:
                article_id = save_article(art["title"], art["link"], art["content"], art["source"], art["published_date"])
                if article_id:
            ...
            print(f"Lỗi xử lý nguồn {src_name}: {e}")
